...
class Vampire:
    ...
    def leech_blood(self, target) -> None:
        ...
        print(f'{self.name} chupa sangre de {target.name} causando {dmg} de daño y se cura {math.floor(dmg/2)}')
    ...
    def hypnosis(self, target) -> None:
        if self.mp >= 5:
            ...
            print(f'{self.name} usa confusion. {target.name} esta confundido')
        else:
...
class Person:
    ...
    def escape(self) -> None:
        ...
        if dice == 20:
            ...
            print('Tratas de correr, pero tu oponente es mucho mas veloz')
...
if __name__ == '__main__':
    ...
    time.sleep(3)
    print(f'SOY {vampire.name.upper()} Y VENGO A CHUPAR TU SANGRE')
